...
// Duplicate from build_memory_index, TODO: Move into header file
...
// Essentially a duplicate of the main method of build_memory_index
int build_mem_index(diskann::Metric& metric, const std::string& index_path_prefix, const std::string& in_memory_data_path, const std::string& data_type, const unsigned R, const unsigned L, const float alpha, int interval,  const unsigned num_threads) {
  std::string data_path = in_memory_data_path + "_" + std::to_string(interval);
...
int search_disk_index(
    ...
    const std::string& in_memory_data_path,
    ...
    const bool use_sq = false, const int num_intervals = 1) {
  ...
  if (mem_L) {
    _pFlashIndex->load_mem_index(metric, query_dim, mem_index_path + "_0", num_threads, mem_L);
  }
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++) {
    ...
    int64_t base_queries = (int64_t) (query_num / num_intervals);
    int64_t remainder = query_num % num_intervals;
    int64_t interval_start = 0;
    if (use_page_search) {
      
      for (int64_t interval = 0; interval < num_intervals; ++interval){
        int64_t interval_end = (interval_start + base_queries) + (interval < remainder) ? 1 : 0;

        if(use_sq){
            for (_s64 i = interval_start; i < (int64_t) interval_end; i++) {
        ...
        if (mem_L) {
          build_mem_index(metric, index_path_prefix, in_memory_data_path, data_type, R, L, alpha, interval, num_threads);
...
int main(int argc, char** argv) {
  std::string data_type, dist_fn, index_path_prefix, result_path_prefix,
      query_file, gt_file, disk_file_path, mem_index_path, in_memory_data_path;
  unsigned              num_threads, K, W, num_nodes_to_cache, search_io_limit, L, R;
  unsigned              mem_L;
  std::vector<unsigned> Lvec;
  bool                  use_reorder_data = false;
  bool                  use_page_search = true;
  float                 use_ratio = 1.0, alpha;
  bool use_sq = false;
  int num_intervals = 1;
  ...
  try {
    ...
    desc.add_options()("in_memory_data_path",
                        po::value<std::string>(&in_memory_data_path)->required(),
                        "Data path prefix for building the different in-memory indices");
    desc.add_options()("num_intervals", po::value<int>(&num_intervals)->default_value(1), 
                       "The number of intervals to split the queries across");
  ...
  try {
    if (data_type == std::string("float"))
      return search_disk_index<float>(metric, index_path_prefix,
                                      ...
                                      disk_file_path, in_memory_data_path, data_type,
                                      num_threads, K, W, num_nodes_to_cache,
                                      search_io_limit, Lvec, mem_L, R, L, alpha, use_page_search, use_ratio, use_reorder_data, use_sq, num_intervals);
      ...
      return search_disk_index<uint8_t>(
          ...
          disk_file_path, in_memory_data_path, data_type, num_threads, K, W, num_nodes_to_cache, search_io_limit, Lvec, mem_L, R, L, alpha,
          use_page_search, use_ratio, use_reorder_data, use_sq, num_intervals);
    else {
